...
DWORD CMyMCIWave::OpenWave(CString strFileName)//打开波形文件
{
    ...
    {
        LogUtility::showMessageInConsole(_T("找不到文件"));
    }
...
DWORD CMyMCIWave::PlayWave()//播放波形文件
{
    ...
    mciPlayParam.dwCallback = (DWORD)AfxGetApp()->GetMainWnd()->m_hWnd;//设置回调函数窗口句柄

    mciSendCommand(m_wDeviceID, MCI_SEEK,MCI_SEEK_TO_START, NULL);
	dwPlayFlag = mciSendCommand(m_wDeviceID, MCI_PLAY, MCI_NOTIFY, (DWORD)(LPVOID)&mciPlayParam);//开始播放数据
	if (dwPlayFlag)//播放失败
